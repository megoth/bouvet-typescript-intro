module.exports = (env, callback) ->
  ...
  getSlides = (contents) ->
    return env.config.slides.map (slide) -> contents.slides[slide].index

  getSlideId = (slide) ->
    parts = slide.filepath.relative.split('/')
    if (parts.length == 1)
      parts = slide.filepath.relative.split('\\')
    return parts.slice(-2, -1)

  env.helpers.getSlides = getSlides
  env.helpers.getSlideId = getSlideId
